...
import { addressSchema } from '@/app/schemas/addressSchema'; // Caso tenha um schema de validação
...
export function Address({
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    try {
      addressSchema.parse(updatedData)
      setErrors({})
  ...
  const handleSave = async () => {
    try {
      console.log('Enviando endereço:', { ...data, employeeId })
      await AddressService.createAddress({ ...data, employeeId: employeeId })
      onNext()
    } catch (error) {
  ...
  return (
    <div className="p-8 bg-gray-800 rounded-lg shadow-md space-y-3 w-full">
      ...
      {[
        ...
        { name: 'zipCode', placeholder: 'Digite o CEP', label: 'CEP' }, // Corrigido nome
      ].map((field) => (
