...
import { UserResponse, userResponseSchema } from '@/app/schemas/userSchema'
...
const UserList = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-green-900 to-green-600">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {userList.map((user, index) => (
                  <tr key={user.id} className={`hover:bg-gray-600 transition-all duration-200 ${!user.active ? 'bg-gray-500 text-gray-400' : 'text-white'}`}>
                    <td className="px-4 py-3 border border-gray-600">{index + 1}</td>
                    <td className="px-4 py-3 border border-gray-600">{user.name || 'Não informado'}</td>
                    ...
                    <td className="px-4 py-3 border border-gray-600 space-x-2">
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg ${user.active ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'} text-white`}
                      >
