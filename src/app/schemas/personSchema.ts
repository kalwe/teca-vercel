...
const formatCPF = (value: string) => {
  ...
  return digitsOnly
    ...
    .slice(0, 14); // Garante que não ultrapasse o tamanho correto
};
...
export const personSchema = z.object({
  ...
  taxId: z
    .string()
    .min(11, "O CPF deve conter pelo menos 11 dígitos numéricos.") // Permite CPF sem máscara
    .max(14, "O CPF deve conter no máximo 14 caracteres.") // Evita CPFs maiores que o esperado
  ...
  dateOfBirth: z
    ...
    .transform((date) => {
      return new Date(date.replace(/-/g, "/")); // Converte para `Date` e permite `dd-mm-aaaa` ou `dd/mm/aaaa`
    }),
